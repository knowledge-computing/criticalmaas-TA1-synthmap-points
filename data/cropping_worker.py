...
def cropping_worker(map_id, file_name, data_dir, solutiona_dir, crop_legend):
    ...
    print(' == Cropping on map:', file_name)
    ...
    if os.path.exists(out_file_path0):
        print('Find an existing mask: ', out_file_path0, '\n')
        selected_map_for_examination = cv2.imread(out_file_path0, cv2.IMREAD_GRAYSCALE)
        return selected_map_for_examination == 0 
    ...
    return selected_map_for_examination[:, :, 0] == 0    